...
namespace TFD
{
	...
	Init_Code InitSDK(int procID, uintptr_t moduleBase, uintptr_t moduleSize)
	{
		try
		{
			...
			//uintptr_t LineTraceMulti = moduleBase + 0x361BFF0;
			uintptr_t LineTraceMulti = SearchSignature(procID, moduleBase, moduleSize, LineTraceMultiSig, LineTraceMultiMask);
			if (LineTraceMulti)
			{
				LineTraceMulti = (moduleBase + LineTraceMulti) - 0x66;
				...
				std::cout << "[Cheat] Failed to find UM1TargetStatics::LineTraceMulti! \n";
			...
			do
			{
				...
				{
					if (world->OwningGameInstance && world->OwningGameInstance->LocalPlayers.IsValid() && world->OwningGameInstance->LocalPlayers[0]->ViewportClient)
					{
						TFD::UGameViewportClient* ViewportClient = world->OwningGameInstance->LocalPlayers[0]->ViewportClient;
						// Grab vtable
						void** vtable = *(void***)ViewportClient;

						// Change memory protection so we can write
						DWORD oldProtect;
						VirtualProtect(&vtable[112], sizeof(void*), PAGE_EXECUTE_READWRITE, &oldProtect);

						// Save original
						Cheat::oPostRender = (decltype(Cheat::oPostRender))vtable[112];

						// Swap
						vtable[112] = (void*)&Cheat::PostRender;

						// Restore protection
						VirtualProtect(&vtable[112], sizeof(void*), oldProtect, &oldProtect);
			...
			CFG::LoadCFG();
			SavePointCFG::InitializeSavePointSystem();

#ifdef IS_DEBUG_VERSION
			std::cout << "[Cheat] Save point system initialized.\n";
#endif
